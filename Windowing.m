function [Coef,target] = frame(data,classCodes,winSize,pDelay)
...
%
%  This file uses tdfeatsE, lpc, and enframe
%
...
for class = 1:nclass
    ...
    GoodData = data(classCodes==class,:); %Only choose data that is 'on'
    
    for s = 1:sensors
        % Partition the data into a matrix of nWin columns of analysis windows of length winSize
        Fdata = enframe(GoodData(:,s),winSize,pDelay)';
        if isempty(Fdata)
            continue
        end
%         init = Fdata(1,:);
...
%         datamean = mean(Fdata);
%         stddev = std(Fdata);
%         feats = [init' final' minimum' maximum' datamean' stddev'];
        ...
        classCoef = [classCoef,Fdata]; %#ok<AGROW>
    end
    nWin = size(Fdata,2);
    totwin = totwin + nWin;
